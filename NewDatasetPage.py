...
class NewDatasetSidebar(Sidebar):
    def __init__(self, *args,  **kwargs):
        ...
        self.capture_button = ct.CTkButton(
            self, text="Capture", command=lambda: self.capture(self.error_label))
...
class NewDatasetContent(Content):
    ...
    def capture_data(self, label):
        if self.label:
            self.logging_csv(self.label_list.index(
                self.label), self.relative_list)
        else:
        ...
        if ret:
            ...
            if results.multi_hand_landmarks is not None:
                for hand_landmarks, handedness in zip(results.multi_hand_landmarks, results.multi_handedness):
                    ...
                    # normalize (step 4)
                    self.relative_list = self.calc_relative_landmarks(
                        landmark_list)
